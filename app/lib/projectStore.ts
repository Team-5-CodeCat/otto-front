...
// 프로젝트 생성
export function createProject(data: Omit<Project, 'id' | 'webhookUrl' | 'status' | 'createdAt' | 'updatedAt'>): Project {
    const projects = getProjects();
...
// 프로젝트 업데이트
export function updateProject(id: string, data: Partial<Omit<Project, 'id' | 'webhookUrl' | 'createdAt'>>): Project | null {
    const projects = getProjects();
